...
------------------------------------------------------------------------
Fortunately for the MySQL40 users, UT99 Guy  was kind enough to share the solution:
...
CREATE TABLE IF NOT EXISTS `phpbb_calendar_events` (
  ...
  `group_id_list` blob NOT NULL default ',',
  ...
  `rsvp_yes` mediumint(8) UNSIGNED DEFAULT '0' NOT NULL,
  `rsvp_no` mediumint(8) UNSIGNED DEFAULT '0' NOT NULL,
  `rsvp_maybe` mediumint(8) UNSIGNED DEFAULT '0' NOT NULL,
  `recurr_id` mediumint(8) UNSIGNED DEFAULT '0' NOT NULL,
...
but have not tested the edits as I don't have a MySQL40 server on which to test.  
Also the new tables for 0.1.0 (shown below) have yet to be translated for MySQL40.
